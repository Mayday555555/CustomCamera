...
typedef NS_ENUM(NSInteger, CropDragType) {
    CropDragTypeNone,
    CropDragTypeLeftTop,
    CropDragTypeLeftBottom,
    CropDragTypeRightTop,
    CropDragTypeRightBottom,
    CropDragTypeLeft,
    CropDragTypeTop,
    CropDragTypeRight,
    CropDragTypeBottom,
...
/** 4个角落的视图 */
@property (nonatomic, strong)CropCornerView *leftTopCorner;
@property (nonatomic, strong)CropCornerView *rightTopCorner;
@property (nonatomic, strong)CropCornerView *leftBottomCorner;
@property (nonatomic, strong)CropCornerView *rightBottomCorner;

/** 左右两边垂直居中的视图 */
@property (nonatomic, strong)UIView *leftVerticalCenterView;
@property (nonatomic, strong)UIView *rightVerticalCenterView;
/** 实际显示裁剪部分的视图 */
...
- (instancetype)initWithFrame:(CGRect)frame{
    // 处理某些情况 真机进入 CALayer bounds contains NaN: [0 0; nan 200]问题
    ...
    // 默认缩放类型
    _scaleType = CropScaleTypeCustom;
...
- (void)p_addAllViews {
    ...
    _leftVerticalCenterView = [UIView new];
    _rightVerticalCenterView = [UIView new];
    
    
    _leftTopCorner = [[CropCornerView alloc] initWithPosition:CropCornerPositionLeftTop];
    _rightTopCorner = [[CropCornerView alloc] initWithPosition:CropCornerPositionRightTop];
    _leftBottomCorner = [[CropCornerView alloc] initWithPosition:CropCornerPositionLeftBottom];
    _rightBottomCorner = [[CropCornerView alloc] initWithPosition:CropCornerPositionRightBottom];
    ...
    [_containerView addSubview:_leftVerticalCenterView];
    [_containerView addSubview:_rightVerticalCenterView];
}
...
- (void)p_configAllViews {
    ...
    for (UIView *maskView in @[_leftVerticalCenterView, _rightVerticalCenterView]) {
        maskView.backgroundColor = [UIColor whiteColor];
        maskView.layer.cornerRadius = 2.0;
...
- (void)p_addAllConstraints {
    ...
    // 左右垂直居中对齐视图
    [_leftVerticalCenterView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(_containerView);
        make.left.equalTo(_containerView).offset(-2);
        make.size.mas_equalTo(CGSizeMake(4, 20));
    }];
    [_rightVerticalCenterView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(_containerView);
...
/** 根据位置计算出拖动的类型 */
- (CropDragType)p_dragTypeWithLocation:(CGPoint)location {
    ...
    if (x >= 0 && x <= range && y >= 0 && y <= range) {
        ...
        _rightPriority = 250;
        _bottomPriority = 250;
        return CropDragTypeRightBottom;
        ...
        _rightPriority = 250;
        _bottomPriority = 250;
        return CropDragTypeCenter;
    }
}

/** 具体拖动的实现 */
- (void)p_dragCropViewWithType:(CropDragType)dragType gesture:(UIPanGestureRecognizer *)sender {
    
    CGPoint translation = [sender translationInView:sender.view];
    if (_scaleType != CropScaleTypeCustom && (dragType == CropDragTypeLeft || dragType == CropDragTypeTop || dragType == CropDragTypeRight || dragType == CropDragTypeBottom)) {
        dragType = CropDragTypeCenter;
    }
    switch (dragType) {
        case CropDragTypeLeftTop:
            _leftOffset += translation.x;
            _topOffset += translation.y;
            break;
        case CropDragTypeLeftBottom:
            _leftOffset += translation.x;
            _bottomOffset += translation.y;
            break;
        case CropDragTypeRightTop:
            _rightOffset += translation.x;
            _topOffset += translation.y;
            break;
        case CropDragTypeRightBottom:
            _rightOffset += translation.x;
            _bottomOffset += translation.y;
            break;
        case CropDragTypeLeft:
            _leftOffset += translation.x;
            if (_leftOffset < 0) {
                _leftOffset = 0;
            }
            break;
        case CropDragTypeTop:
            _topOffset += translation.y;
            break;
        case CropDragTypeRight:
            _rightOffset += translation.x;
            break;
        case CropDragTypeBottom:
            _bottomOffset += translation.y;
            break;
        case CropDragTypeCenter:
            _leftOffset += translation.x;
            _topOffset += translation.y;
            _rightOffset += translation.x;
            _bottomOffset += translation.y;
...
- (void)setScaleType:(CropScaleType)scaleType {
    ...
    switch (scaleType) {
        case CropScaleTypeCustom:
            // 如果是自定义模式，直接取消了长宽比的约束后即可返回
            return;
        case CropScaleTypeOriginal:
            mutiplyValue = self.frame.size.height/self.frame.size.width;
            break;
        case CropScaleType1To1:
            mutiplyValue = 1.0;
            break;
        case CropScaleType3To2:
            mutiplyValue = 3.0/2.0;
            break;
        case CropScaleType2To3:
            mutiplyValue = 2.0/3.0;
            break;
        case CropScaleType4To3:
            mutiplyValue = 4.0/3.0;
            break;
        case CropScaleType3To4:
            mutiplyValue = 3.0/4.0;
            break;
        case CropScaleType16To9:
            mutiplyValue = 16.0/9.0;
            break;
        case CropScaleType9To16:
